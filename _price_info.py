...
# 필요한 모델 가져오기
from apt.models import Apt_purchase, Apt_jeonse, Apt_list, Apt_current

def create_mean_price():
    # Apt_list와 Apt_detail 데이터를 불러옴

    apt_price = Apt_list.objects.values("complexNo", "cortarNo", "roadAddressPrefix")
    apt_jeonse = Apt_detail.objects.values('complexNo', 'complexno_py', 'supplyAreaDouble')
